...
function updateHtmlFile(filePath) {
    try {
        ...
        // Add responsive wrapper classes to main content if not already present
...
// Main execution
console.log('🚀 Applying modern CSS styling to root HTML files...\n');

// Find all HTML files in root directory (excluding index.html which we already updated)
const rootHtmlFiles = fs.readdirSync('.')
    .filter(file => (file.endsWith('.html') || file.endsWith('.htm')) && file !== 'index.html')
    .map(file => path.join('.', file));

console.log(`Found ${rootHtmlFiles.length} HTML files in root directory to process\n`);
...
console.log(`\n✅ Completed! Updated ${updatedCount} root files with modern styling.`);
console.log(`📁 All HTML files now use /assets/css/main.css for responsive design.`); 